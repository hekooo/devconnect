/*
  # Add Story Likes and Views Functionality

  1. New Features
    - Create story_likes table if it doesn't exist
    - Add helper functions for story likes and views
    - Add RLS policies for story interactions

  2. Security
    - Enable RLS on story_likes table
    - Add policies for viewing, creating, and deleting likes
...
-- Create function to get story likes count
CREATE OR REPLACE FUNCTION get_story_likes_count(story_id uuid)
RETURNS integer
LANGUAGE sql
STABLE
AS $$
...
-- Create function to get story views count
CREATE OR REPLACE FUNCTION get_story_views_count(story_id uuid)
RETURNS integer
LANGUAGE sql
STABLE
AS $$
...
-- Create function to check if user has liked a story
CREATE OR REPLACE FUNCTION has_user_liked_story(story_id uuid, user_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
AS $$
